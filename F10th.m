...
for s=1:S
    ...
    for d=Sd:Ed
        ...
        [xA,A,rA,PM]=OpO(k,nn,Resolution,PortRet,0);% ALPM Sample
        [xP,PmA,rP]=OpO(k,nn,Resolution,PortRet,1); % PMP Sample
...
Fxl=AppData(mat2dataset(x.','VarNames',Asset_Names),dataset(A.',r.','VarNames',{'ALPM','Return'}));
...
if exist('Rt','var')
    Fxl=AppData(mat2dataset(xFinal,'VarNames',Asset_Names),dataset(ALPM,Rt,'VarNames',{'ALPM','Return'}));
...
%***********************************************  Sample Biulder
function [x,A,r,PM]=OpO(k,nn,Resolution,PortRet,isPmp)
...
if isempty(rSam0)
    ...
    while(1)
        ...
        [x,A0,r]=OpO(k,nn,[],fRP(xfinal),isPmp);%Simul(xfinal,nn);
