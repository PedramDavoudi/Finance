...
if  ~exist('Asset_Names','var')
    ...
    for i=1:k
        ...
        Asset_Names_P{i}=['w' num2str(i) '_PMP'];
    end
else
     Asset_Names_P=Asset_Names;
    for i=1:k
        Asset_Names{i}=['WeightOf' Asset_Names{i}];
         Asset_Names_P{i}=['WeightOf' Asset_Names{i} '_PMP'];
    end
end
% Exporting Simulation Data
Fxl=[mat2dataset(x.','VarNames',Asset_Names),dataset(A.',r.','VarNames',{'ALPM','Return'})];
%export(Fxl,'xlsfile',['out\' SenarioName '\Siml.xlsx']);
export(Fxl,'file',['out\' SenarioName '\Siml.txt']);
clear Fxl
% Exporting Simulation Data Efficiency Curve

if exist('Rt','var')
    Fxl=[mat2dataset(xFinal,'VarNames',Asset_Names_P),dataset(ALPM,Rt,'VarNames',{'ALPM','Return'})];
...
% Fxl=cell(k+1+6,2);
if exist('PM','var')
    Fxl=[Fxl,mat2dataset(PM.MV.W,'VarNames',Asset_Names), dataset(PM.MV.A,PM.MV.R,'VarNames',{'Mean_Var_ALPM' 'Mean_Var_Return'})];
    Fxl=[Fxl, dataset(PM.CV.A,PM.CV.R,PM.MAD.A,PM.MAD.R,'VarNames',{'CVaR_ALPM' 'CVaR_Return' 'Absolute_Deviation_ALPM' 'Absolute_Deviation_Return'})];
...
%****************************************** Graph Creater
function Grph(JustSort,x,A,r,SenarioName,Asset_Names,PM,Rt,ALPM,xFinal)
global r1
...
for i=1:k
    try %#ok<TRYNC>
        ...
        saveas(gcf,['out\' SenarioName '\Kr' Asset_Names{i} '.bmp'])
        close gcf
    end
end
for i=1:k
    try
        ...
        ylabel('Return');
        xlabel(['Weight of ' Asset_Names{i}]);
        
        hold off
        saveas(gcf,['out\' SenarioName '\wR' Asset_Names{i} '.bmp'])
        close gcf
        %------------------------------------------------------
        figure();
        hold on
        ...
        xlabel(['Weight of ' Asset_Names{i}]);
        hold off
        saveas(gcf,['out\' SenarioName '\wA' Asset_Names{i} '.bmp'])
        close gcf
    end
end
...
x=[];
for i=1:size(x00,2)
    [x0,~,A0,r0,~]=Simul(x00(:,i),nn);
    x=[x,x0];
    A=[A,A0];
    r=[r,r0];
end
% if JustSimulation==1
x0 = randfixedsum(k,nn,100,0,100);
A0=nan(1,nn);
% y=A;
r0=A0;
% [~,A0(1),r0(1)]=f(xfinal);% Add the final point to Sapmle collection
% x=[xfinal,x];
for i=1:nn
    [~,A0(i),r0(i)]=f(x0(:,i));
end
